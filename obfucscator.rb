...
class Obfuscator < String
  attr_reader :unified_word
  
  def initialize(word, level)
    @word = word
     if level > 6
      raise "only up to level 6"
    end
    @unified_word = obfusc(@word, level)
  end
...
private
  ...
  def rand_from_level(char, level)
    ...
    a = code[char][level].length > 1 ? choices[Math.random(choices.length)] : choices[0]
...
p Obfuscator.new("fresh", 1).unified_word
